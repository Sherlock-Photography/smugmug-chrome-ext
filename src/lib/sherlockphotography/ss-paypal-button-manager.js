YUI.add('ss-paypal-button-manager', function(Y, NAME) {
	var PayPalButtonManager = Y.Base.create(NAME, Y.Base, [], {}, {
		...
		renderPayPalButtons : function(code, item_name, item_number) {
			...
			if (result.test("form")) {
				result = this._renderPayPalButton(result, item_name, item_number);
			} else {
				result.all('form').each(function(form) {
					form.replace(that._renderPayPalButton(form, item_name, item_number));
				});
			}
		 ...
		 * @param item_number
		 */
		_renderPayPalButton: function(form, item_name, item_number) {
			var button;
